...
import com.sunac.ow.owdomain.EsInfoParamInfo;
...
public class EsInfoParamInfoCoProcessFunction extends KeyedCoProcessFunction<String, AllData, EsInfoParamInfo, AllData> {
    ...
    private static final Logger log = LoggerFactory.getLogger(EsInfoParamInfoCoProcessFunction.class);
    transient Connection connection = null;
    @Override
    public void open(Configuration parameters) throws Exception {
        RuntimeContext runtimeContext = getRuntimeContext();
        mapState = runtimeContext.getMapState(new MapStateDescriptor<>("map_stat", String.class, EsInfoParamInfo.class));
    ...
    public void processElement1(AllData allData, KeyedCoProcessFunction<String, AllData, EsInfoParamInfo, AllData>.Context context, Collector<AllData> collector) throws Exception {
        ...
        if (StringUtils.isNotBlank(fldGuid)) {
            Tuple2<Connection, Boolean> tp = JdbcUtils.insertOrIgnore2(connection,"idx_owe_es_info_param_info", fldGuid, allData.getFld_reason_guid());
            connection = tp.f0;
    ...
    public void processElement2(EsInfoParamInfo newData, KeyedCoProcessFunction<String, AllData, EsInfoParamInfo, AllData>.Context context, Collector<AllData> collector) throws Exception {
        ...
        if (isSend) {
            String sql = JdbcUtils.makeSQL(new Tuple2<>(Config.EsInfoParamInfo, newData));
            Tuple2<Connection, Boolean> tp2 = JdbcUtils.queryByTabAndKey2(context, connection,"idx_owe_es_info_param_info", fldGuid, sql);
            connection = tp2.f0;
